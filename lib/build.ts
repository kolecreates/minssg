...
const importTransformation = {
  extensions: [".html"],
  process: (content: string, filePath: string, templatesDir: string) => {
    ...
    if (match) {
      ...
      if (fs.existsSync(templatePath)) {
        ...
        for (const [key, value] of Object.entries(layoutOptions)) {
          ...
          layoutContent = layoutContent.replace(regex, value as string);
        }
...
export function build(publicDir: string, buildDir: string, templatesDir: string) {
