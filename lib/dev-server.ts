...
export function startDevServer(publicDir: string, buildDir: string, templatesDir: string) {
  // Inject refresh script into HTML files
  function injectRefreshScript(content: string) {
    const script = `
      <script>
  ...
  async function onChange(event: string, filename: string | null) {
